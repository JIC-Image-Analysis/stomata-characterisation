...
def find_opening(image_collection, stomata_id, timepoint):
    ...
    line = p2 - p1
    m = line.magnitude
    p1_inner = p1 + line * (cut1.x / line.magnitude)
    p2_inner = p1 + line * (cut2.x / line.magnitude)
...
# Loop over z-stacks and plot images...
    ...
    for z, proxy_im in enumerate(z_iter):
        ...
        cv2.line(im, p1.astype("int").astuple(), p2.astype("int").astuple(), (0, 255, 0), 1)
        cv2.line(im, p1_inner.astype("int").astuple(), p2_inner.astype("int").astuple(), (255, 0, 0), 1)
